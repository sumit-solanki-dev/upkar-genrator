async function a(){let r=[];try{r=(await(await fetch("/products.json")).json()).slice(0,4)}catch(e){console.error("Failed to fetch featured products:",e)}const t=document.createElement("section");return t.className="featured-products",t.dataset.featuredProducts="section",t.innerHTML=`
          ...
          ${r.map(e=>`
              <article class="featured-product-card swiper-slide">
                ...
                <div class="featured-product-card__body">
                  <p class="featured-product-card__model">${e.model}</p>
  ...
  `,t}export{a as createFeaturedProductsSection};
