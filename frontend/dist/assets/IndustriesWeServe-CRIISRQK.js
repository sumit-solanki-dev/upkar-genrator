const i=[{name:"Construction",icon:`
    ...
    `}];function t(){const a=document.createElement("section");return a.className="industries",a.dataset.industries="section",a.innerHTML=`
      ...
      <div class="industries__grid" aria-label="Industries served by UPKAR Generator">
        ${i.map(e=>`
            <article class="industry-card" data-industry-card>
              <div class="industry-card__bg" style="background-image: url('/images/industries/${e.name.toLowerCase()}.webp')"></div>
              <div class="industry-card__overlay"></div>
              <span class="industry-card__icon">${e.icon}</span>
              <h3 class="industry-card__title">${e.name}</h3>
            </article>
          `).join("")}
      </div>
    </div>
  `,a}export{t as createIndustriesWeServeSection};
